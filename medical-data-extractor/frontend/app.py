    ...
    with col2:
        if st.session_state:
            ...
            if file_format == "prescription":
                name = st.text_input(label="Name", value=st.session_state.get("patient_name", "John Smith"))
                address = st.text_input(label="Address", value=st.session_state.get("patient_address", "162 Example St, M"))
                medicines = st.text_input(label="Medicines", value=st.session_state.get("medicines", "Betaloc"))
                directions = st.text_input(label="Directions", value=st.session_state.get("directions", "100 mg 1 tab BID"))
                refill = st.text_input(label="Doctor", value=st.session_state.get("refill", "Steve Johnson"))
